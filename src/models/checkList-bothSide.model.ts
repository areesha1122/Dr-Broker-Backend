...
import { CHECKLIST_BOTHSIDE_DOCUMENT } from '../utils/check-both-side-enum';
import { v4 as uuidv4 } from 'uuid';

// Create an interface that uses the enum values as keys with boolean values
export interface IChecklistBothSide extends Document {
  uuid: string;
  key: boolean;
...
const ChecklistBothSide = new Schema<IChecklistBothSide>({
  // Initialize all checklist items to false by default
  uuid: { type: String, default: uuidv4, unique: true },
  ...Object.values(CHECKLIST_BOTHSIDE_DOCUMENT).reduce((acc:any, key:string) => {
...
export default mongoose.model<IChecklistBothSide>('ChecklistBothSide', ChecklistBothSide);