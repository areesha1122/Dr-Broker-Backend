...
import { CHECKLIST_BUYSIDE_DOCUMENT } from '../utils/check-buy-side-enum';
import { v4 as uuidv4 } from 'uuid';

// Create an interface that uses the enum values as keys with boolean values
export interface IChecklistBuySide extends Document {
  uuid: string;
  key: boolean;
...
const ChecklistBuySideSchema = new Schema<IChecklistBuySide>({
  // Initialize all checklist items to false by default
  uuid: { type: String, default: uuidv4, unique: true },
  ...Object.values(CHECKLIST_BUYSIDE_DOCUMENT).reduce((acc:any, key:string) => {
...
export default mongoose.model<IChecklistBuySide>('ChecklistBuySide', ChecklistBuySideSchema);